...
CFStringRef get_device_full_name(const AMDeviceRef device) {
    ...
    if (device_name != NULL) {
        full_name = CFStringCreateWithFormat(NULL, NULL, CFSTR("%@ (%@, %@, %@)"), device_udid, model_name, product_version, build_version);
    } else {
...
void usage(const char* app) {
    NSLog(
        ...
        @"  -M, --mobileprovision        mobile provision file path\n"
...
int main(int argc, char *argv[]) {
    ...
    static struct option longopts[] = {
        ...
        { "mobileprovision", required_argument, NULL, 'M'},
    ...
    while ((ch = getopt_long(argc, argv, "VmcdvunrILefFD:R:X:i:b:a:t:p:M:1:2:o:l:w:9BWjNs:OE:CA:k:S:P", longopts, NULL)) != -1)
    {
        switch (ch) {
        case 'm':
            install = false;
